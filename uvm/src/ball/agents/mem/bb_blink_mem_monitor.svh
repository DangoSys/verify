...
class bb_mmio_read_monitor #(int IN_BW = 1, int OUT_BW = 1) extends uvm_monitor;
  `uvm_component_param_utils(bb_mmio_read_monitor#(IN_BW, OUT_BW))
  typedef virtual bb_blink_if#(IN_BW, OUT_BW) vif_t;
  vif_t vif;
  uvm_analysis_port #(bb_mmio_read_item) read_ap;
...
class bb_mmio_write_monitor #(int IN_BW = 1, int OUT_BW = 1) extends uvm_monitor;
  `uvm_component_param_utils(bb_mmio_write_monitor#(IN_BW, OUT_BW))
  typedef virtual bb_blink_if#(IN_BW, OUT_BW) vif_t;
  vif_t vif;
  uvm_analysis_port #(bb_mmio_write_item) write_ap;
